...
func _ready() -> void:
	...
	if editor != null and suelo != null and mueble != null:
		...
		mueble.visible = true
		mueble.process_mode = Node.PROCESS_MODE_INHERIT
		mueble.set_meta("layout_oculto_por_boton", false)
		...
		_comprobar(is_zero_approx(suelo.self_modulate.a), "ocultar la capa base vuelve transparente el suelo")
		...
		_comprobar(is_equal_approx(suelo.self_modulate.a, 1.0), "restaurar la capa base restaura su opacidad")
	if muro != null:
